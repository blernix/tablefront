    ...
    <TourProvider steps={DASHBOARD_TOUR_STEPS}>
      <div className="space-y-6 p-4 md:p-6">
        ...
        {restaurant.subscription?.status === 'past_due' && (
          <div className="rounded-2xl border border-amber-300 bg-amber-50 p-4 md:rounded-xl md:p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[15px] font-semibold text-amber-800 md:text-base">Paiement en attente</p>
                <p className="mt-1 text-[13px] text-amber-700 md:text-sm">
                  Votre période d&apos;essai est terminée. Ajoutez un moyen de paiement pour continuer à utiliser TableMaster sans interruption.
                </p>
                <button
                  onClick={handleManageSubscription}
                  disabled={isManagingSubscription}
                  className="mt-3 inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-amber-600 text-white text-[13px] font-medium hover:bg-amber-700 transition-colors md:text-sm"
                >
                  {isManagingSubscription ? 'Chargement...' : 'Ajouter un moyen de paiement'}
                </button>
              </div>
            </div>
          </div>
        ...
        {restaurant.subscription?.status === 'trial' && restaurant.subscription?.trialEndsAt && (
          <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 md:rounded-xl md:p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[15px] font-semibold text-blue-800 md:text-base">Période d&apos;essai en cours</p>
                <p className="mt-1 text-[13px] text-blue-700 md:text-sm">
                  Votre essai gratuit se termine le {new Date(restaurant.subscription.trialEndsAt).toLocaleDateString('fr-FR')}. Ajoutez un moyen de paiement avant cette date pour éviter toute interruption.
                </p>
              </div>
            </div>
          </div>
